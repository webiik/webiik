...
class Router
{
    ...
    /** @var array All defined routes with names, uris and available methods */
    private $routes = [];

    /** @var array URI masks of routes for fast duplicates recognition */
    private $routeUris = [];

    /** @var array Names of routes for fast duplicates recognition */
    private $routeNames = [];

    /** @var array Err routes handler names */
    private $errRouteHandlers = ['404' => null, '405' => null];

    /** @var callable Custom logger */
    private $logger;
    ...
    /**
     * Return HTTP status or false before match() method calling
     * @return bool|mixed
     */
    public function getStatus()
    {
        return isset($this->routeInfo['http_status']) ? $this->routeInfo['http_status'] : false;
    }

    /**
     * Map handler for error route
     * @param string $handler
     */
    public function map404($handler)
    {
        $this->errRouteHandlers['404'] = $handler;
    }
     ...
     * @param string $handler
     */
    public function map405($handler)
    {
        $this->errRouteHandlers['405'] = $handler;
    }
    ...
    public function getUriFor($routeName, $lang = false, $params = [])
    {
                    ...
                    if ($route['uri'] != '/') {
                        ...
                        if ((($paramsCount >= $reqParamsCount) && ($paramsCount <= $reqParamsCount + $optParamsCount)) || (($paramsCount >= $reqParamsCount) && $wildParamsCount > 0)) {
                            ...
                            foreach ($params as $givenParam) {
                                ...
                                if (isset($routeParams[$i]['condName'])) {
                                    ...
                                    if (!preg_match('/' . $conditionRegex . '/', $givenParam)) {
                                        $msg = [
                                            'Executed route name' => isset($this->routeInfo['name']) ? $this->routeInfo['name'] : '',
                                            'Class' => 'Router',
                                            'Method' => 'getUriFor',
                                            'Invalid route name' => $routeName,
                                            'Param' => $givenParam,
                                            'Regex' => $conditionRegex,
                                            'Message' => 'Parameter \'' . $givenParam . '\' must match the following regex \'/' . $conditionRegex . '/\'.',
                                        ];
                                        $this->log($msg);
                                        return false;
                                    }
                            ...
                            $msg = [
                                ...
                                'Method' => 'getUriFor',
                                'Invalid route name' => $routeName,
                                'Requiered parameters count' => $reqParamsCount,
                                'Optional params count' => $optParamsCount,
                                'Wildcard params count' => $wildParamsCount,
                                'Given params count' => $paramsCount,
                                'Message' => 'Route with name \'' . $routeName . '\' has \'' . $reqParamsCount . '\' required, \'' . $optParamsCount . '\' optional and \'' . $wildParamsCount . '\' wildcard parameters, but received only \'' . $paramsCount . '\' parameter(s).',
                            ];
                            $this->log($msg);
                            return false;
                        }
        ...
        $msg = [
            ...
            'Invalid route name' => $routeName,
            'Language' => $lang,
            'Message' => 'Route with name \'' . $routeName . '\' does not exist in language \'' . $lang . '\'.',
        ];
        $this->log($msg);
        return false;
    }
        ...
        foreach ($this->routes[$this->lang] as $routeId => $route) {
            ...
            if (preg_match('/^' . $routeRegexMask . '$/', $request_uri, $requestUri)) {
                ...
                if (!$methodMatch) {
                    ...
                    $this->routeInfo['handler'] = $this->errRouteHandlers['405'];
                    break;
                }
